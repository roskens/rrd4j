...
class RrdTuneCmd extends RrdToolCmd {
	...
	private void tuneHeartbeat(RrdDb rrd, String heartbeatStr) throws IllegalArgumentException, IOException {
	...
	private void tuneMinimum(RrdDb rrd, String minimumStr) throws IllegalArgumentException, IOException {
	...
	private void tuneMaximum(RrdDb rrd, String maximumStr) throws IllegalArgumentException, IOException {
	...
	private void tuneName(RrdDb rrd, String nameStr) throws IllegalArgumentException, IOException {
	...
	private void tuneType(RrdDb rrd, String typeStr) throws IllegalArgumentException, IOException {
		...
		Datasource ds = rrd.getDatasource(dsName);
		ds.setDsType(DsType.valueOf(dsType));
	}

	private void dump(RrdDb rrd) throws IllegalArgumentException, IOException {
		StringBuffer line = new StringBuffer();
		int n = rrd.getDsCount();
		for (int i = 0; i < n; i++) {
			Datasource ds = rrd.getDatasource(i);
			line.append("DS[");
			line.append(ds.getName());
			line.append("] typ: ");
			line.append(ds.getType());
