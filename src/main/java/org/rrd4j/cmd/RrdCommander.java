...
public class RrdCommander {
	 ...
	 */
	public static synchronized Object execute(String command) throws IOException, IllegalArgumentException {
	...
	public static synchronized Object execute(List<String> args) throws IOException, IllegalArgumentException {
                String command = args.get(0);
		for (RrdToolCmd rrdCommand : rrdCommands) {
			if (command.equals(rrdCommand.getCmdType())) {
				return rrdCommand.executeCommand(args);
			}
		}
		throw new IllegalArgumentException("Unknown RRDTool command: " + command);
	}
